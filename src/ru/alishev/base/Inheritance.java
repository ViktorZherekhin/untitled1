...
public class Inheritance {
    public static void main(String[] args) {
        Animal anim1 = new Animal();           // мы уже создавали ранее этот класс (см. ru.alishev.base.ClassesAndObjects.java),
                                                // поэтому можем создать объект на его базе
        anim1.eat();            // таким образом наше животное умеет есть и спать
        anim1.sleep();

        Dog dog1 = new Dog();   // после того, как мы указали что класс "ru.alishev.base.Dog" является наследником класса "ru.alishev.base.Animal"
        dog1.eat();             // мы можем применить к объекту класса "ru.alishev.base.Dog" те же методы
        dog1.sleep();           // но для "ru.alishev.base.Dog" вызовется переопределнный метод eat()
        dog1.gavgav();
        ...
        // автоматически унаследуются от родительского класса "ru.alishev.base.Animal"
        ...
        Animal anim2 = new Dog();      // это называется "Upcasting"
                                        // здесь объект воспринимаем как родителя "ru.alishev.base.Dog", т.е. получается
        ...
        // например:
        Dog dog4 = (Dog) anim1;
        dog4.gavgav();             // здесь будет ощибка "class ru.alishev.base.Animal cannot be cast to class ru.alishev.base.Dog"
