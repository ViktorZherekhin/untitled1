...
import ru.alishev.base.Interfaces.Info2;
...
public class ClassesAndObjects {   // это главный класс (публичный). Отмечен модификатором "public"
    ...
    public static void main(String[] args) {
        // создадим несколько людей на базе класса "ru.alishev.base.Person":
        ...
        // Прдставим, что мы хотим переопределить метод "eat()" в классе "ru.alishev.base.Animal"
        ...
        // Т.е. создадим объект класса "ru.alishev.base.Animal" и там в нем переопределим метод:

        Animal anim2 = new Animal(){            // здесь мы, на самом деле, создаем АНОНИМНЫЙ класс,
            public void eat(){                  // который наследуется от "ru.alishev.base.Animal"
                System.out.println("Переопределили метод eat()!");
            }                                   // и который переопределяет метод "eat()"
        };                                      // т.е. в "anim2" лежит объект-наследник класса "ru.alishev.base.Animal"
        anim2.eat();

        // Альтернативный способ (но опять же надо создавать целый класс - "ru.alishev.base.Animal3"):
        // - добавить имя переопределяемого метода в интерфейс "Info"
        // - создать класс, который его реализует ("class ru.alishev.base.Animal3 implements Info"), см. ниже
        // - добавить следующие две команды:
        Info2 drink1 = new Animal3();
        drink1.drinking();                  // выведется "ru.alishev.base.Animal is drinking water!"
        ...
        Info2 drink2 = new Info2() {
            ...
            public void drinking() {                    // т.е. нам предлагается самим реализоавть метод "eat()"
                System.out.println("No comment...");  // в Интерфейсе при создании объекта Интерфеса
                                                    ...
                                                    // реализовать (переопредлить) метод интерфейса "Info2"
...
class Person {   // это вспомогательный класс (их может быть много)
                // у класса могут быть данные (поля, свойства): возраст, цвет глаз, имя, и т.д.
...
class Animal {
    ...
    private String animalColor = "yellow";      // к этой переменной у класса-наследника "ru.alishev.base.Dog" уже не будет доступа
                                                // это свойство объекта приватное, поэтому извне к нему обратиться
                                                // или изменить мы не можем. Но можем задать его с помощью
                                                // Конструктора (т.е. единожды, при создании объекта класса "ru.alishev.base.Animal")
                                                // Это удобно, например, при создании соединения к БД.
                                                // т.е. в качестве приватного свойства задать IP-адрес,
                                                // чтобы нигде в программе никто не мог его изменить случайно/нарочно
                                                // и программа перестала бы работать



    public void eat(){
        System.out.println("ru.alishev.base.Animal is eating");
    }
    public void sleep(){
        System.out.println("ru.alishev.base.Animal is sleeping");
    }
    public void makeSound() {
        System.out.println("ru.alishev.base.Animal makes common sound");
    }

}


class Dog extends Animal {   // класс "ru.alishev.base.Dog" более узкое понятие, чем "ru.alishev.base.Animal"
                            // этим мы записали, что класс "ru.alishev.base.Dog" наследуется от класса "ru.alishev.base.Animal",
                            // т.е. класс "ru.alishev.base.Animal" стал родителем (суперклассом) класса "ru.alishev.base.Dog" (подкласса)
    ...
    public void eat(){          // переопределили родительский метод
        System.out.println("ru.alishev.base.Dog is eating...");
    }
    public void showName(){
        System.out.println(animalName);     // поле "animalName" так же унаследовано от родительского класса
    }
    public void makeSound(){
        System.out.println("ru.alishev.base.Dog says: gav-gav!");
    }
}

class Animal2 {             // для урока по ru.alishev.base.Generics создал
    private int id;
    ...
    public void eat(){                              // создали метод, специфичный только для "ru.alishev.base.Animal2"
...
class Animal3 implements Info2 {
    ...
    public void drinking() {
        System.out.println("ru.alishev.base.Animal is drinking water!");
